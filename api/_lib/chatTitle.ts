...
import { API_SOLUTION, OSAII_CHAT_URL, nuclearProviderChain } from './apiSolution.js';
...
interface Hop { provider: 'eaon' | 'cerebras' | 'osaii'; model: string }

/** Cheapest first; the chain only exists so a provider outage costs nothing visible. */
const HOPS: Hop[] = API_SOLUTION === 'nuclear' ? nuclearProviderChain('default').map(hop => ({
  provider: 'osaii', model: hop.model,
})) : [
...
async function askHop<T>(hop: Hop, messages: Array<{ role: string; content: string }>, system: string, parse: (raw: string) => T | null): Promise<T | null> {
  const key = hop.provider === 'osaii' ? (process.env.OSAII_API_KEY || '').trim()
    : hop.provider === 'eaon' ? (process.env.EAON_API_KEY || '').trim()
    : (process.env.CEREBRAS_API_KEY || '').trim();
  if (!key && hop.provider !== 'osaii') return null;
  if (isProviderTripped(hop.provider) && hop.provider !== 'osaii') return null;

  const url = hop.provider === 'osaii' ? OSAII_CHAT_URL : hop.provider === 'eaon'
  ...
  const body: Record<string, unknown> = {
    ...
    temperature: 0.3,
    max_tokens: hop.provider === 'osaii' ? 512 : 120,
    stream: false,
  };
